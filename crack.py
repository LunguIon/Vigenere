...
num = dict(zip(range(26), string.ascii_lowercase))  # for reverse mapping: numbers to letter

A = [0.08167, 0.01492, 0.02782, 0.04253, 0.12702, 0.02228, 0.02015, 0.06094, 0.06966, 0.00153, 0.00772, 0.04025,
     0.02406, 0.06749, 0.07507, 0.01929, 0.00095, 0.05987, 0.06327, 0.09056, 0.02758, 0.00978, 0.0236, 0.0015, 0.01974,
     0.00074]

a = input('Please enter your Vigenere cipher text:').lower()
a = list(a)
a1 = a
c = 0
d = []
...
while k <= 20:  # gives result of first k number of coincidences
    ...
    for i, j in zip(a, a1):  # zip(a,b) makes [(a0,b0),(a1,b1)] for all the element of a and b
        if i == j:  # for loop compares and increments c if the same element is found in a1 and a2
            c += 1  # c is a reference variable that contains the number of the same elements
    print('Number of coincidences for', k, 'shift is:', c)
    d.append(c)
    k += 1
    c = 0

L = max(d)  # for the highest number in the list
